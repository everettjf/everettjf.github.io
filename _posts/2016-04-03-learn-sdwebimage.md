---
layout: post
title: "SDWebImage Source Code Reading Notes"
title_zh: "SDWebImage 源码阅读笔记"
lang_original: zh
categories: Skill
comments: true
---
...
# Basic Information
 ...
 - Description : 
A classic library for asynchronously downloading images, with caching.
...
Handy dispatch_main, guaranteeing it runs on the main thread.
...
Determine whether a certain class exists.
...
Dynamically call a singleton method.
...
An advantage of NSOperation over GCD: you can configure the maximum number of concurrent threads.
...
This is how you dynamically configure the executing Class.
...
A very convenient synchronization method on a concurrent queue.
...
Each URL corresponds to multiple callbacks. The same URL may be requested multiple times simultaneously, avoiding duplicate downloads.
...
Start the download. If it's Last In First Out, set the dependency of the last added task to the task about to be added.
...
In the background, guarantee the task is cancelled.
...
The NSURLConnection that initiates the download operation.
...
Start the download. NSRunLoopRun() starts the runloop on the background thread. NSURLConnection depends on the RunLoop.
...
First cancel the Connection, then stop the RunLoop.
...
On 304, directly return the image from the cache.
...
Received data. self.imageData is allocated in didReceiveResponse.
...
Progressively load the image... draw the progressively-rendered image in memory.
...
Regarding Orientation, see this article <http://feihu.me/blog/2015/how-to-handle-image-orientation-on-iOS/>
...
Clear the cache on a memory warning.
...
FOUNDATION_STATIC_INLINE is worth learning and using.
The way to compute an image's cost.
...
[self new] — learned it. Worth investigating the difference from [XXXX new].
...
The in-memory cache is NSCache.
...
When caching to disk and recalculate is needed, determine whether it's png. Two methods for determining png:

- Whether it has an alpha (transparency) channel.
- Whether it has png's magic-number prefix.

PS: If you need to load a gif, be careful not to recalculate here, otherwise after loading from disk again it's no longer a gif file (it becomes a jpg file).
...
The sharedManager is thread-safe.
...
Scale the image. When storing locally, store the scaled image.
...
Determine the image type from the file header.
...
Use CGImageSourceCreateWithData to read a Gif file.
...
decode image, to optimize image loading speed.
...
Use the file system's stored metadata about files (time, size, etc.) to clean up files.
...
KeyType and ObjectType. So Objective-C has this syntax too. Similar to generics.
...
The declaration of NSDictionary. Covariance and contravariance in generics.

See this article: <http://blog.sunnyxx.com/2015/06/12/objc-new-features-in-2015/>
...
Correct the argument type. Determine the argument type. NSNull.
...
NSArray actually has this method—Objective-C really is developer-friendly.
...
Associate an operation set with each UIView. (Mainly used when a Cell is reused, or when loading a url image repeatedly onto a UIImageView, to cancel the previous loading task.)
...
Looks like the new version of SDWebImage integrated an indicator feature.
...
Cancel the previous loading operation on the current imageview.
