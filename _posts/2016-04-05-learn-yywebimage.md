---
layout: post
title: "YYWebImage Source Code Reading Notes"
title_zh: "YYWebImage 源码阅读笔记"
lang_original: zh
categories: Skill
comments: true
---
...
# Basic Information
 ...
 - Description : 
One of the YYKit components. A freshly-baked WebImage.
...
CGImageGetHeight and CGImageGetBytesPerRow.
...
appex : Share Extension. A feature added in iOS8.
...
The dispatch_semaphore_xxx series. It even uses static variables here. But they're only valid within the file, so it doesn't matter.
...
The NSProxy protocol, which implements the NSObject protocol.
...
The network thread. Used by NSURLConnection.
...
On iOS 8 and below, use dispatch_set_target_queue to set the priority.

Create as many queues as there are active processors. And pick queues one by one.

    int32_t cur = OSAtomicIncrement32(&counter); atomic increment.
...
Initiate the request on the specified thread.
...
Can you also determine a file url this way? How do you determine a url on the network?
...
There are quite a lot of progressive checks here. The implementation is still fairly concise.
...
Can control whether to show the network activity indicator. The network indicator in the status bar.
...
Play sprite-sheet animations. Nice.
...
This file has 2k+ lines of code... so meticulous.
...
This setter approach is nice. It first cancels the operation currently running on the ImageView.
...
Ensure sync on the main thread.
...
Check whether the sentinel number has changed.
...
Pass in the sentinel. It represents the current operation.
...
If the passed-in sentinel isn't the previous sentinel, it means there was a duplicate call.
