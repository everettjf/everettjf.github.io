...
appex : Share Extension. iOS 8 new feature.
...
dispatch_semaphore_xxx series. Here actually used static variables. But effective within file, doesn't matter.
...
NSProxy protocol, implements NSObject protocol
...
Network thread. For NSURLConnection use.
...
iOS 8 and below use dispatch_set_target_queue to set priority.

Create queues equal to number of active processors. And select queues one by one.
...
Start request on specified thread.
...
Can check file url this way? How to check network URLs?
...
Here progressive has many checks. Implementation is quite concise.
...
Can control whether to show network activity indicator. Network indicator in status bar.
...
This file has 2k+ lines of code... detailed
...
This setter writing style is good. First cancel current ImageView's executing operation.
...
Check if sentinel number changed
...
Pass in sentinel. Represents current operation.
...
If passed sentinel is not the previous sentinel, it means duplicate call.
