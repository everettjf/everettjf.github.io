---
layout: post
title: "YYCache Source Code Reading Notes"
title_zh: "YYCache 源码阅读笔记"
lang_original: zh
categories: Skill
comments: true
---
...
Here is the author's introduction to the design ideas: <http://blog.ibireme.com/2015/10/26/yycache/>

<!-- more -->


# Global Note
The author's growth experience is worth learning from; he improved very quickly during just over a year of iOS development.
...
```
  ...
  - YYMemoryCache is the memory cache.
 - Keys are retained, not copied.
 - Uses the LRU (least-recently-used) algorithm.
...
The QuartzCore framework is worth studying.
#TODO#
...
This is how you can check whether a certain header file is included.
...
cost and time (the last-used time).
_prev and _next are modified with __unsafe_unretained; they are retained by the outer CFMutableDictionaryRef.

The @package modifier: accessible within the framework, but not accessible from outside the framework.
...
Definition of the doubly linked list. Head, tail. dic is used to store elements while also retaining them.
...
Marking the usage of CFMutableDictionaryRef.
...
Basic operations of a doubly linked list. Haven't looked at algorithms for a long time. Just pasting it here.
...
The old version used OSSpinLock here; it has now been switched to pthread_mutex_lock. For the reason, see the author's article,
...
Here it uses trylock, and when the lock can't be acquired, it usleeps.
...
Listen for entering the background or memory warnings.
...
Note the lock. And set the latest access time of the accessed element. And move this element to the head of the doubly linked list. The time complexity here is constant.

Learned a function for getting the time: CACurrentMediaTime().
...
pthread_main_np determines whether it's the main thread. If it returns non-zero, it's the main thread.
...
Listen for the app entering the background, as well as system low-memory.
...
static inline: file-local, inlined.
...
YYDiskCache checks this threshold; below 20k it's stored in sqlite, otherwise it's stored as a file.
...
Inside, you can directly shadow the outer self, which is convenient to use.
...
I don't know about shm and wal. (Seems like I've always just used sqlite, but never studied it carefully.)
...
Creating an index (never used this in 5 years of C++).

primary key(key) — you can specify the sqlite type afterwards like this.
...
Usage of CFDictionaryCreateMutable. Usage of Core Foundation.
...
(?1, ?2, ?3, ?4, ?5, ?6, ?7) 
There can be numbers after the question marks……
...
Update the access times of multiple keys.
...
Here a new FileManager is created. No delegate or such is set here. ?? Why create a new FileManager?
...
```
 ...
 - Each time, get 16 of the least-recently-accessed entries.
 - Delete them one by one (file or database record), checking whether the remaining size meets the target size.

PS: The Desc here is a bug. It should be Asc. See: <https://github.com/ibireme/YYCache/issues/37>
