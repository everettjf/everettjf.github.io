---
layout: post
title: "Implementing a Simple Thread Pool in C++"
title_zh: "C++ 实现一个简单的线程池"
lang_original: zh
categories:
  - guide
tags:
...
When optimizing launch, try to keep work off the main thread whenever possible. When moving work to background threads, also try to avoid heavy concurrency — do it one by one, giving the main thread more chances to run. One way to achieve this is to use a thread pool.
...
This 2012 article <http://progsch.net/wordpress/?p=81> describes a simple implementation written in C++11. The implementation below makes a few minor optimizations on top of the code from that article.
...
There are many ways to create threads on iOS; the above is just one C++ implementation. There are other simple implementations on GitHub too:
