---
layout: post
title: "UITableView-FDTemplateLayoutCell Source Code Reading Notes"
title_zh: "UITableView-FDTemplateLayoutCell 源码阅读笔记"
lang_original: zh
categories: Skill
comments: true
---
...
# Basic Information
 ...
 - Description : 
Simple and easy-to-use automatic UITableViewCell height.
The author's blog article: <http://blog.sunnyxx.com/2015/05/17/cell-height-calculation/>

<!-- more -->

# Global Note
Simple and easy to use, but when used in some complex interfaces (e.g., chat windows), you still need to consider more optimization issues.
...
`__kindof XXXClass` can be used like this.
...
Use a UITableViewCell template cell to calculate height; via `fd_isTemplateLayoutCell` you can determine inside the cell whether the current one is a template cell. This lets you skip some operations unrelated to height.
...
An implementation that adds a property using the runtime.

The SEL-typed `_cmd` exists inside every method and represents the method itself.
Therefore, you can use `NSStringFromSelector(_cmd)` to return the current method's name.

Using the getter's SEL (i.e., `_cmd`) as the key for `objc_getAssociatedObject` is worth learning. But note that the setter must also use the same key, i.e., `@selector(fd_isTemplateLayoutCell)`.
...
A way to define an array with designated indices. ObjC really has quite a few little tricks.
...
The AutoLayout method for computing Size: `systemLayoutSizeFittingSize`.
Here it adds a width constraint, computes the height, and then removes it. A nice idea.
...
Without AutoLayout, use `sizeThatFits` to get the size. A custom cell needs to implement this method.
...
- Each CellIdentifier is a key.
- Used to store template cells.
- The template cell is used to build a cell in memory and compute its height.
...
Cache portrait and landscape separately.
...
I don't understand this one. NSMethodSignature.
...
A clever trick. It seems this can make the "hint" method name show up in the stack backtrace.
...
The magical load method. When I first saw the load method, I marveled that Objective-C is so flexible.

The load documentation is as follows:
...
`method_exchangeImplementations` can swap two Methods.

Similar to API HOOK in Windows development (detours, mhook), Objective-C's runtime provides this out of the box, at a higher level.

Commonly known as "swizzle method".
...
Add-on features are recommended to be added via Category in this way.
